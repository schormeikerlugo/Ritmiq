...
export function createPairRequest(db, {
  ...
  // Auto-pair per cuenta Supabase.
  if (supabaseUserId) {
    const sibling = db.prepare(
      "SELECT 1 FROM devices WHERE supabase_user_id = ? AND status = 'approved' LIMIT 1"
    ).get(supabaseUserId);
    if (sibling) {
      const token = approveDevice(db, {
        deviceId, displayName, supabaseUserId, cookiesBlob,
      });
      return { status: 'approved', deviceToken: token, displayName };
    }
  }
