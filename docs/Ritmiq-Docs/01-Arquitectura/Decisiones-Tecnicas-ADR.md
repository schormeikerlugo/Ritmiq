...
- **Adenda (robustez, 2026-08)**: el player se "congelaba" al reproducir una canción nueva (más tras volver de segundo plano); workaround del usuario: pisar otra canción y volver. Causa: `backend.load()` sin timeout → en un stall de iOS (endpoint stale del cache de reachability 30s, cold-start del túnel) la promesa nunca resolvía → `await` colgado; y las refs `loadedTrackId/Fingerprint` se marcaban antes de `play()` OK y no se limpiaban en error → los atajos bloqueaban re-tocar la misma canción. Fix: (1) `load()` con timeout 13s + recuperación de `stalled`/`waiting`; (2) refs marcadas solo tras `play()` exitoso y limpiadas en el catch; (3) invalidar el cache de reachability en `visibilitychange` (foreground); (4) **auto-reintento** una vez ante fallo. Ver `html-audio-backend.js` `load()`, `use-player.js` `loadAndPlayCurrent`.

## ADR-036 — Compartido dinámico de playlists: visibilidad, jalado y notificaciones

- **Contexto**: compartir playlists solo existía como snapshot JSON al inbox, y el botón "Guardar" del inbox estaba **roto** (llamaba `createPlaylist`/`addTracksToPlaylist` inexistentes y pasaba objetos donde se esperaban trackIds). No había forma de que otros descubrieran/"jalaran" tus playlists desde tu perfil, ni control de visibilidad, ni notificaciones.
- **Decisión**:
  1. **Visibilidad por playlist** (`playlists.visibility` = `private`|`friends`|`public`, default `private`). Índice parcial para las expuestas. Selector en el menú de PlaylistView + badge en el header. Favoritas siempre privada.
  2. **RLS de lectura cruzada** (sin tocar la política owner): política "visible read" en `playlists` y `playlist_tracks` — `public` para cualquier autenticado, `friends` si `are_mutual_friends(auth.uid(), owner)` (función `security definer` sobre la vista `mutual_friends`). La metadata de `tracks` **NO** se abre por RLS: se resuelve vía Edge con service-role para no exponer la biblioteca completa del dueño.
  3. **Edge `get-profile-playlists`**: valida amistad, devuelve solo las playlists visibles del `ownerId` con sus tracks como snapshot (`{ytId,title,artist,coverUrl,durationSeconds}`). Una sola llamada.
  4. **Jalado = copia independiente** (`pull-playlist.js` `pullPlaylistSnapshot`): respeta la FK `playlist_tracks.track_id→tracks.id` — primero materializa cada track en la biblioteca (`library.addFromMeta`, idempotente por `(user_id,yt_id)`), recolecta UUIDs, `create(name, {source*})`, `addTracks`. Guarda `source_playlist_id`/`source_owner_id`. Se reutiliza para **arreglar el inbox** y para el jalado desde el perfil.
  5. **Re-sync** (botón "Actualizar desde el original"): sólo en copias con `source_*`. Re-obtiene el snapshot vía `get-profile-playlists` y hace **MERGE aditivo** (`addTracks` dedup) sobre la misma playlist — no borra tus cambios.
  6. **Notificaciones in-app** (tabla `notifications` con realtime): Edge `notify-playlist-pulled` inserta con service-role (anti-falsificación) tras revalidar visibilidad/amistad + dedup 1h. Store social (`loadNotifications`, `markNotificationRead`, `markAllNotificationsRead`, `deleteNotification`), canal realtime nuevo, carga al login, y **pestaña "Actividad"** en FriendsView (campana + badge, auto-marca leído al abrir). Push best-effort al dueño reusando `send-push-notification`.
- **Consecuencias**: los amigos descubren y copian playlists desde el perfil respetando la visibilidad; el dueño se entera con una bandeja de actividad; el inbox roto queda arreglado. Modelo de copia (no suscripción en vivo) → cero realtime cruzado, RLS simple; el re-sync cubre la actualización manual. Migración `20260901000000_playlist_sharing.sql` aplicada a prod; Edges `get-profile-playlists` + `notify-playlist-pulled` desplegadas; SQLite desktop con columnas idempotentes. Verificado E2E (playlist pública devuelta con tracks, notificación creada). Builds PWA+desktop verdes. Ver [[social]].
- **Adenda (fixes post-lanzamiento)**: (1) **playlist fantasma** al ver un perfil ajeno — la RLS de lectura cruzada en `playlist_tracks` hacía que el canal realtime SIN filtro recibiera eventos de playlists ajenas; se quitó esa RLS (la Edge resuelve con service-role) + guard `isMine` en `applyRemotePlaylistTrack`. (2) **Pantalla negra al eliminar** — `performRemovePlaylist` navegaba después de borrar; ahora navega antes y `remove()` es optimista (best-effort remoto). (3) **Borrado que reaparecía** — la cola offline conservaba un `playlist.upsert`/`add` que resucitaba lo borrado al drenarse; `purgeQueueForPlaylist(Track)` lo purga antes del delete. (4) **Carátula no se copiaba** al jalar — `create(name, {coverUrl})` + el helper propaga `coverUrl`.

## ADR-037 — Capacidades iOS (Fases A/B/C) + gesto de "volver"

- **Contexto**: acercar la PWA a una app nativa aprovechando capacidades de iOS Safari (18.4 / 26) y Android que no usábamos. Investigación basada en notas de WebKit (Safari 18.4, 26.0, 26.6) y la doc de Declarative Web Push. Realidad confirmada: **Background Fetch/Sync NO existe en iOS ni en 26** → la descarga con la app cerrada sigue sin ser posible; no se prometió.
- **Decisión**:
  - **Fase A — manifest enrichment** (`vite.config.js`): `shortcuts` (Buscar/Favoritas/Amigos/Descargas vía `?go=`), `share_target` GET (recibir enlaces/texto → búsqueda), `launch_handler: navigate-existing`, `display_override`, `categories`, `screenshots` 1080×1920. `App.jsx` `detectLaunchIntent()` enruta `?go=`/`?shared_url|text` al haber usuario.
  - **Fase B — onboarding iOS 26 + splash**: `share.js` `getIOSMajorVersion()`/`isIOS26OrNewer()` (lee `Version/NN` porque iOS 26 congela el OS en la UA); `IOSInstallHint` menciona "Abrir como web app" en iOS 26+; splash de iPhone 16 Pro/Pro Max añadidos. `black-translucent` se mantiene (correcto para dark edge-to-edge).
  - **Fase C — Declarative Web Push**: la Edge `send-push-notification` emite el formato declarativo estándar (`web_push:8030` + `notification` con `navigate` absoluta + `app_badge`) y CONSERVA los campos planos para el SW imperativo (retrocompat). `sw-push.js` lee ambos y respeta `navigate`. Se arreglaron los deep-links de notificación (`?openTab=` no lo procesaba nadie): `view store` `friendsTab` + `goFriends(tab)`, `App.jsx` enruta al arrancar y escucha `push-click` con la app abierta.
  - **Gesto edge-swipe-back** (`use-edge-swipe-back.js`): swipe desde 24px del borde izquierdo → "atrás" jerárquico (cierra NowPlaying/jam/cola/sidebar y luego `goBack()`). Sólo táctil, sólo horizontal-derecha, con feedback `translateX` en `<main>`. No colisiona con carruseles ni dnd-kit.
- **Consecuencias**: instalación y puntos de entrada más nativos; notificaciones más fiables en iOS 18.4+ (sin depender del SW) y que llevan a la pestaña correcta; navegación con gesto natural. Edge desplegada; builds PWA+desktop verdes; gesto no-op en desktop. Ver [[Capacidades-iOS-Fases-ABC]].
- **Adenda — Web Share Target NO funciona en iOS (`039576b`)**: el `share_target` del manifest es "Limited availability" ([MDN](https://developer.mozilla.org/en-US/docs/Web/Progressive_web_apps/Manifest/Reference/share_target)); **Safari/iOS no lo soporta**, sólo Android. Fallback universal implementado sin depender del share target: (1) `stores/search.js` busca por el **ytId** cuando la query es un enlace/id de YouTube (`extractYtId` exportado + `music.youtube.com`), devolviendo el video exacto; (2) chip **"Pegar link de YouTube copiado"** en `SearchView` que lee `navigator.clipboard.readText()` con la búsqueda vacía. Es el camino real para compartir a Ritmiq en iPhone.
