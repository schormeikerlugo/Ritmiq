...
- 2026-05-29 (UX header desktop): la flecha de cerrar ahora apunta a la **derecha** (`ChevronRight`) en desktop —reflejando que el panel se cierra hacia la derecha (animación `fade-in-right`)— y sigue como `ChevronDown` en móvil (fullscreen que baja). Detección via [[use-mobile-viewport]] (`useMobileViewport(768)`, alineado con el breakpoint `min-width:769px` del CSS). Más aire entre el título y la línea divisoria: `.header` desktop pasa a `height:auto` con `padding-bottom: space-5` (antes `space-3`) y `margin-bottom: space-4`.
- 2026-05-29 (fix solape lyrics): el header desktop ahora es **`position: sticky; top: calc(-1*space-4); z-index:5; background: bg-1`**. El `.root` desktop es el contenedor de scroll (`overflow-y:auto`); con el header sticky + fondo opaco, al activar letras el [[LyricsPanel]] **nunca solapa** el header (la letra pasa siempre por debajo). Resuelve el bug reportado donde "LETRA SINCRONIZADA" se montaba sobre "REPRODUCIENDO/Nirvana". Verificado con screenshot Playwright a viewport ancho (panel 380px). Complementa el `padding-top` que ya tenía el `.panel` del LyricsPanel en desktop.
- 2026-05-31 (**jam guest + sugerir**): nuevo item de menú "Sugerir a la jam" (icono `Radio`,
  visible solo con jam activa) que encola la canción actual en [[jam_queue]] para aprobación del
  host. Si `useJamStore.mode === 'guest'`, los 5 controles de transporte y el scrub se
  **deshabilitan** con hint "El host controla la reproducción" (el guard central de
  [[use-jam-sync]] además revierte cualquier cambio que se cuele). Verificado con Playwright.
