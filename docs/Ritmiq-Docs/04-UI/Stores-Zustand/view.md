...
- 2026-05-29: estado `lyricsOpen` movido a este store (antes local en [[NowPlaying]]) para que el botón de letra del [[Player]] (footer desktop) y el de NowPlaying lo compartan. Acciones `toggleLyrics`, `setLyricsOpen(v)`, `openLyrics()` (abre NowPlaying + letra).
